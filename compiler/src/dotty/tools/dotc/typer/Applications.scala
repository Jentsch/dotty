      ...
      case mt: MethodType if mt.paramInfos.length == 1 =>
        ...
        if (bunchedArgs != Nil && argTypes != Nil) {
          val nameIndex: List[Name] =
            val annotation = unapplyFn.denot.asSymDenotation.getAnnotation(defn.PatternNamesAnnot)

            if annotation.isDefined then
              // TODO: How to handle an 5overloaded unapply?
              // TODO: Why lead nested use of the annotation to a (sometimes unresovoled) recusive type?
              // TODO: Add phase for PatternNamesAnnot to check validity
              val annotationNames = annotation.get

              annotationNames.arguments.head match
                case Typed(seq: SeqLiteral, _) =>
                  seq.elems.map { case Literal(const) => Names.termName(const.stringValue) }
              ...
              report.error(s"${qual.show} doesn't support named arguments", tree.srcPos)
              List.empty
          end nameIndex

          val numberOfAlreadyUsedArguments = nameIndex.length - argTypes.length

          var remainingNames = nameIndex.drop(numberOfAlreadyUsedArguments)
          ...
          // TODO: error on names which refere to positional pattern
